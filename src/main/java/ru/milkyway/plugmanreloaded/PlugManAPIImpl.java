...
public final class PlugManAPIImpl implements PlugManAPI {
    ...
    @Override
    public PluginResult loadPlugin(@NotNull File file) {
        return runSyncIfNeeded(() -> lifecycleManager.load(file));
    }

    @Override
    public PluginResult loadPlugin(@NotNull String pluginName) {
    ...
    @Override
    public PluginResult unloadPlugin(@NotNull Plugin targetPlugin, boolean deep) {
        return runSyncIfNeeded(() -> lifecycleManager.unload(targetPlugin, deep));
    }

    @Override
    public PluginResult unloadPlugin(@NotNull String pluginName, boolean deep) {
        return runSyncIfNeeded(() -> {
    ...
    @Override
    public PluginResult reloadPlugin(@NotNull Plugin targetPlugin) {
        return runSyncIfNeeded(() -> lifecycleManager.reload(targetPlugin));
    }

    @Override
    public PluginResult reloadPlugin(@NotNull String pluginName) {
        return runSyncIfNeeded(() -> {
            Plugin target = lifecycleManager.getPlugin(pluginName);
            if (target == null) {
                return PluginResult.ofError(FailureReason.PLUGIN_NOT_FOUND, "plugin", pluginName);
    ...
    @Override
    public PluginResult restartPlugin(@NotNull Plugin targetPlugin) {
        return runSyncIfNeeded(() -> lifecycleManager.restart(targetPlugin));
    }

    @Override
    public PluginResult restartPlugin(@NotNull String pluginName) {
        return runSyncIfNeeded(() -> {
            Plugin target = lifecycleManager.getPlugin(pluginName);
            if (target == null) {
                return PluginResult.ofError(FailureReason.PLUGIN_NOT_FOUND, "plugin", pluginName);
    ...
    @Override
    public PluginResult enablePlugin(@NotNull Plugin targetPlugin) {
        return runSyncIfNeeded(() -> lifecycleManager.enable(targetPlugin));
    }

    @Override
    public PluginResult enablePlugin(@NotNull String pluginName) {
        return runSyncIfNeeded(() -> {
            Plugin target = lifecycleManager.getPlugin(pluginName);
            if (target == null) {
                return PluginResult.ofError(FailureReason.PLUGIN_NOT_FOUND, "plugin", pluginName);
    ...
    @Override
    public PluginResult disablePlugin(@NotNull Plugin targetPlugin) {
        return runSyncIfNeeded(() -> lifecycleManager.disable(targetPlugin));
    }

    @Override
    public PluginResult disablePlugin(@NotNull String pluginName) {
        return runSyncIfNeeded(() -> {
            Plugin target = lifecycleManager.getPlugin(pluginName);
            if (target == null) {
                return PluginResult.ofError(FailureReason.PLUGIN_NOT_FOUND, "plugin", pluginName);
    ...
    @Override
    public DependencyNode getNode(@NotNull String pluginName) {
        Map<String, DependencyNode> graph = graphManager.buildGraph(true);
        return graph.get(pluginName.toLowerCase(Locale.ROOT));
    }

    @Override
    public Set<String> getDependents(@NotNull String pluginName) {
        return graphManager.getDependents(pluginName);
    }

    @Override
    public Set<String> getDependencies(@NotNull String pluginName) {
    ...
    @Override
    public Optional<PluginInfo> getPluginInfo(@NotNull String pluginName) {
        Plugin target = lifecycleManager.getPlugin(pluginName);
        if (target != null) {
    ...
    @Override
    public Optional<PluginInfo> getPluginInfo(@NotNull Plugin plugin) {
        return Optional.ofNullable(lifecycleManager.getPluginInfo(plugin));
    }

    @Override
    public Optional<File> findJarFile(@NotNull String pluginName) {
    ...
    @Override
    public CompletableFuture<Optional<UpdateInfo>> checkUpdate(@NotNull String pluginName) {
        Plugin target = lifecycleManager.getPlugin(pluginName);
        if (target != null) {
