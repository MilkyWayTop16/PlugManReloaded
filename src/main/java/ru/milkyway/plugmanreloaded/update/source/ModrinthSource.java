...
public final class ModrinthSource implements UpdateSource {
    ...
    public @Nullable ProjectMatch identifyByName(@Nullable PluginIdentity identity) {
        ...
        List<String> queries = PluginMatcher.getSearchAliases(pluginName);
    ...
    private @Nullable ProjectMatch pickSearchHit(PluginIdentity identity, JsonArray hits) {
        String normalizedPlugin = PluginMatcher.normalizeName(identity.pluginName());
        List<String> aliases = PluginMatcher.getSearchAliases(identity.pluginName());
        ...
        for (JsonElement element : hits) {
            ...
            if (PluginMatcher.isCompanion(identity.pluginName(), title)
                    || PluginMatcher.isCompanion(identity.pluginName(), slug)) {
                continue;
            }

            boolean aliasMatch = false;
            for (String alias : aliases) {
                if (alias.equalsIgnoreCase(slug) || PluginMatcher.normalizeName(alias).equalsIgnoreCase(PluginMatcher.normalizeName(slug))) {
                    aliasMatch = true;
                    break;
                }
            }
            ...
            if (!aliasMatch) {
                double titleScore = PluginMatcher.resourceNameSimilarity(identity.pluginName(), title);
                double slugScore = PluginMatcher.similarity(normalizedPlugin, PluginMatcher.normalizeName(slug));
                ...
                if (identity.authors() != null && !identity.authors().isEmpty() && remoteAuthor != null && !remoteAuthor.isBlank()) {
                    if (!PluginMatcher.isAuthorCompatible(identity.authors(), remoteAuthor) && !brandMatch) {
    ...
    private @Nullable JsonObject pickFile(@Nullable JsonArray files, String targetName) {
        ...
        for (JsonElement element : files) {
            ...
            if (PluginMatcher.isNonRuntimeArtifact(filename)) {
                continue;
            }
            if (PluginMatcher.isCompanion(targetName, filename)) {
                continue;
            }

            double score = PluginMatcher.similarity(targetName, filename) + PluginMatcher.platformBonus(filename);
