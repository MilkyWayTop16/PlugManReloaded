...
public final class HangarSource implements UpdateSource {
    ...
    public @Nullable ProjectMatch identifyByName(@Nullable PluginIdentity identity) {
        ...
        if (title != null && PluginMatcher.isCompanion(identity.pluginName(), title)) {
            return null;
        }
    ...
    private boolean corroborated(PluginIdentity identity, JsonObject project, String slug, String title) {
        ...
        if (PluginMatcher.authorsMatch(identity.authors(), owner)) {
            return true;
        }
        if (PluginMatcher.hasBrandCorroboration(identity, slug, owner, title, string(project, "description"))) {
            return true;
        }
        if (identity.authors() != null && !identity.authors().isEmpty() && owner != null && !owner.isBlank()) {
            if (!PluginMatcher.isAuthorCompatible(identity.authors(), owner)) {
                return false;
            }
        }
        return PluginMatcher.isDistinctive(PluginMatcher.normalizeName(identity.pluginName()))
    ...
    private @Nullable JsonObject resolveProject(PluginIdentity identity) {
        ...
        for (String alias : PluginMatcher.getSearchAliases(pluginName)) {
