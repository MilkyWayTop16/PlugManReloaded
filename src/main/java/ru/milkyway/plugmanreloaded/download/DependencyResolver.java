...
public class DependencyResolver {
    ...
    private boolean nameBelongsToDependency(String depName, @Nullable SearchResultEntry hit) {
        ...
        if (PluginMatcher.isCompanion(depName, hit.title())) return false;
        if (PluginMatcher.resourceNameSimilarity(depName, hit.title()) >= MIN_NAME_SIMILARITY) return true;
        String slug = hit.projectId();
        return slug != null && !PluginMatcher.isCompanion(depName, slug)
                && PluginMatcher.resourceNameSimilarity(depName, slug) >= MIN_NAME_SIMILARITY;
    }
}
